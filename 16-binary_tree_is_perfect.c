...
unsigned char is leaf(const binary_tree_t *node);
size_t depth(const binary_tree_t *tree);
const binary_tree_t get leaf(const binary_tree_t *tree);
int is perfect_recursive(const binary_tree_t *tree, size_t leaf depth, size_t level); 
...
int is_perfect_recursive(const binary_tree_t *tree, size_t leaf_depth, size_t level)
{
	...
	if (tree->lef= NULL 1 tree->right == NULL)
		return (0);
	return (is_perfect_recursive (tree->left, leaf_depth, level 1) &&
		is_perfect_recursive (tree->right, leaf_depth, level + 1));

/**
 * binary_tree_is perfect - Checks if a binary tree is perfect.
 * @tree: A pointer to the root node of the tree to check.
